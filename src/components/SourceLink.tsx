...
export function SourceLink({ source, locale }: { source: Source; locale: Locale }) {
  ...
  return (
    <a className="source-link" href={source.url} target="_blank" rel="noreferrer">
      <span>{source.publisher} — {source.title}</span><small>{dates}{source.access !== 'available' && ` · ${source.access === 'unavailable' ? (locale === 'tr' ? 'Erişilemiyor' : 'Unavailable') : (locale === 'tr' ? 'Yerini yeni kaynak aldı' : 'Superseded')}`}</small>
