import { LayerEvidence } from '../../components/ClaimEvidence'
...
export function ComparisonMatrix({ locale, solutions }: { locale: Locale; solutions: Solution[] }) {
  const rows: Row[] = [
    { label: locale === 'tr' ? 'Amaç ve kapsam' : 'Purpose and scope', render: (solution) => solution.summary[locale] },
    ...
    ...harnessLayers.map((layer): Row => ({ label: getLocalizedText(layerNames[layer], locale), render: (solution) => <><StatusMark status={solution.layers[layer].state}>{labelFor(layerStateLabels, solution.layers[layer].state, locale)}</StatusMark>{solution.layers[layer].claimIds.length > 0 && <LayerEvidence claimIds={solution.layers[layer].claimIds} locale={locale} />}</> })),
