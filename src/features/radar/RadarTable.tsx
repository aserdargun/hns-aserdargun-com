...
import { harnessLayers, type EvidenceKind, type Locale, type Solution } from '../../content/schema'
import { getLocalizedText } from '../../content/selectors'
import { evidenceKinds } from './radar-state'

const layerNames = { execution: { tr: 'Çalıştırma', en: 'Execution' }, tools: { tr: 'Araçlar', en: 'Tools' }, context: { tr: 'Bağlam', en: 'Context' }, lifecycle: { tr: 'Yaşam döngüsü', en: 'Lifecycle' }, observability: { tr: 'Gözlemlenebilirlik', en: 'Observability' }, verification: { tr: 'Doğrulama', en: 'Verification' }, governance: { tr: 'Yönetişim', en: 'Governance' } }
const evidenceLabels: Record<EvidenceKind, { tr: string; en: string }> = { evidence: { tr: 'Kanıt', en: 'Evidence' }, synthesis: { tr: 'Sentez', en: 'Synthesis' }, 'watch-signal': { tr: 'İzleme sinyali', en: 'Watch signal' } }
...
export function LayerCoverage({ solution, locale }: { solution: Solution; locale: Locale }) {
  const label = harnessLayers.map((layer) => `${getLocalizedText(layerNames[layer], locale)}: ${solution.layers[layer].state}`).join(', ')
  return <div className="layer-coverage" role="img" aria-label={label}>{harnessLayers.map((layer) => <span key={layer} className={`layer-${solution.layers[layer].state}`} title={`${getLocalizedText(layerNames[layer], locale)}: ${solution.layers[layer].state}`} />)}</div>
}

export function EvidenceCell({ solution, locale }: { solution: Solution; locale: Locale }) {
  return <div className="evidence-kinds">{evidenceKinds(solution).map((kind) => <StatusMark key={kind} status={kind}>{getLocalizedText(evidenceLabels[kind], locale)}</StatusMark>)}</div>
}
...
export function RadarTable({ locale, solutions, selected, onToggle }: Props) {
  return <div className="radar-table-wrap"><div className="layer-legend" aria-label={locale === 'tr' ? 'Harness katmanları' : 'Harness layers'}>{harnessLayers.map((layer) => <span key={layer}>{getLocalizedText(layerNames[layer], locale)}</span>)}</div><table className="radar-table"><thead><tr><th><span className="sr-only">{locale === 'tr' ? 'Seç' : 'Select'}</span></th><th>{locale === 'tr' ? 'Çözüm' : 'Solution'}</th><th>{locale === 'tr' ? 'Sınıf' : 'Class'}</th><th>{locale === 'tr' ? 'Katman kapsamı' : 'Layer coverage'}</th><th>{locale === 'tr' ? 'Olgunluk' : 'Maturity'}</th><th>Radar</th><th>{locale === 'tr' ? 'İncelendi' : 'Reviewed'}</th><th>{locale === 'tr' ? 'Kanıt' : 'Evidence'}</th></tr></thead><tbody>{solutions.map((solution) => <tr key={solution.id} className={selected.has(solution.id) ? 'is-selected' : ''}>
    ...
    <th scope="row"><a href={solution.canonicalUrl} target="_blank" rel="noreferrer">{solution.name} ↗</a><small>{solution.organization}</small></th><td>{words(solution.class)}</td><td><LayerCoverage solution={solution} locale={locale} /></td><td>{words(solution.maturity)}</td><td><StatusMark status={solution.radar}>{solution.radar.toUpperCase()}</StatusMark></td><td>{solution.lastReviewedAt}</td><td><EvidenceCell solution={solution} locale={locale} /><EvidenceDisclosure solution={solution} locale={locale} /></td>
  </tr>)}</tbody></table></div>
}
